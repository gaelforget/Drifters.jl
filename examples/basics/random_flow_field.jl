...
# ╔═╡ 7b2aa76b-faa4-4bca-b154-25b9a31f5293
begin
using JLD2
function GulfMexico(fil="GulfMexico_CloudDrift_mean.jld2")
	fil="/Users/gforget/mywork/projects_major/2019-jlpkgs/OceanRobots.jl/docs/paper_JuliaCon2023/figs/"*fil
	load(fil)
end
end
...
# ╔═╡ cba71f2d-7635-421c-b63a-246a3c8cf3de
GM=GulfMexico(); GM_median=GulfMexico("GulfMexico_CloudDrift_median.jld2")

# ╔═╡ 2960982b-01ee-4f34-a8a4-6d135718c782
heatmap(GM["u"])

# ╔═╡ 9f6e3545-fc3d-40a2-8b21-24e795d460dd
heatmap(GM_median["u"])
...
# ╔═╡ 881ce568-5bd5-478e-ba45-0e7a46e15890


# ╔═╡ 1efeeb4a-15f3-4644-ba35-c91d912ad3e5
begin
	gdf=IndividualDisplacements.groupby(𝐼.🔴,:ID)
	nnt=length(unique(𝐼.🔴.t))
end

# ╔═╡ 0df81d14-d6ae-42f1-b048-af485e47eba0
fig0=begin
	fi=Figure(); ax=Axis(fi[1,1])
	m=ones(size(gdf[1].x))
	for D in gdf
		x=D.x; y=D.y; m.=1.0
		t=(abs.(diff(x)).>10).||(abs.(diff(y)).>10)
		sum(t)>0 ? m[findall(t.>0)[1]:end].=NaN : nothing
		lines!(x.*m,y.*m)
	end
#	[lines!(D.x,D.y) for D in tmp1]
	fi
end
...
begin
	...
	time = Observable(nt)
	xp=@lift( 🔴_by_t[$time].x )
	yp=@lift( 🔴_by_t[$time].y )
...
begin
	if false
		...
		record(fig,fil, 1:10:nt; framerate = 20) do t
			time[] = t
		end
		LocalResource(fil)
	else
...
manifest_format = "2.0"
project_hash = "855e857c58dd7d9399010dba6c9990f1535b876d"
...
git-tree-sha1 = "b4ce591b91f500126c9c5030aa5b8a710c7db7a0"
...
git-tree-sha1 = "95d85e8f98769da4d539ed5578c1c6bf1c7792b6"
